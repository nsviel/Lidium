...
struct ImDrawList;                  // A single draw command list (generally one per window, conceptually you may see this as a dynamic "cloud" builder)
...
// - To use 16-bit indices + allow large cloudes: backend need to set 'io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset' and handle ImDrawCmd::VtxOffset (recommended).
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset  = 1 << 3    // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large cloudes (64K+ vertices) while still using 16-bit indices.
};
...
//   those fields allow us to render cloudes larger than 64K vertices while keeping 16-bit indices.
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start offset in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support cloudes larger than 64K vertices with 16-bit indices.
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AllowVtxOffset          = 1 << 3   // Can emit 'VtxOffset > 0' to allow large cloudes. Set when 'ImGuiBackendFlags_RendererHasVtxOffset' is enabled.
};
