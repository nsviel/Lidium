...
public:
  //Bundle functions
  void compute_bundleByClass(Mesh* mesh, float step);
  void compute_bundleByClass_maxAngle(Mesh* mesh_in, int stepAngle, int maxAngle);
  ...
  //Subfunctions
  void make_checking(Mesh* mesh);
  void make_clear();
  void make_bundle(Mesh* mesh, float stepAngle, int maxAngle);
  void make_StudentLaw();

  //Plot functions
  void plot_confidenceInterval();
  void plot_intensityBundle(Mesh* mesh);
